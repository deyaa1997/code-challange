  ...
  return (
    <section className={styles["repo-section"]}>
      <div className={styles["repo-main"]}>
        <h1 className={styles["repo-text"]}>
          <span className={styles["repo-label"]}>name</span>: {repoData.name}
        </h1>

        <section>
          <Link href={repoData.url} target="_blank" rel="noopener noreferrer">
            <div className={styles["repo-link-container"]}>
              <p>Link</p>
              ...
              <svg
                ...
                className={styles["repo-link-icon"]}
              >
                <g fill="#4b6a9b">
        ...
        <h4 className={styles["repo-text"]} style={{ margin: "15px 0" }}>
          <span className={styles["repo-label"]}>File Type </span>:{" "}
          {repoData.fileType.length ? repoData.fileType.join(", ") : "No File"}
        </h4>

        {repoData.language && (
          <h4 className={styles["repo-text"]} style={{ margin: "15px 0" }}>
            <span className={styles["repo-label"]}>language </span>:{" "}
            {repoData.language}
          </h4>
        )}

        {repoData.forkers.length ? (
          <>
            <h4 className={styles["repo-text"]} style={{ margin: "15px 0" }}>
              <span className={styles["repo-label"]}>Last Three Forkers :</span>
            </h4>
            <div
              style={{
                display: "flex",
            ...
            >
              {repoData.forkers.map((forker) => (
                ...
                >
                  ...
                  <div>
                    <h4 className={styles["repo-text"]}>{forker.name}</h4>
                    <Link
                      href={forker.forkUrl}
                      target="_blank"
